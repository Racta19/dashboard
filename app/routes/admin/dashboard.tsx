...
import { getTripsByTravelStyle, getUserAndTripStats, getUserGrowthPerDay } from '~/appwrite/dashboard'
import { getAllTrips } from '~/appwrite/trips'
import { parseTripData } from '~/lib/utils'
import { Category, ChartComponent, ColumnSeries, DataLabel, Inject, Legend, SeriesCollectionDirective, SeriesDirective, SplineSeries, SplineAreaSeries, Tooltip } from '@syncfusion/ej2-react-charts'
...
const dashboard = ({loaderData}: Route.ComponentProps) => {  
  const user = loaderData.user as User | null | undefined;
    ...
    <main className='dashboard wrapper'>
      ...
      <section className='flex flex-col gap-6'>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6 w-full'>
          <StatsCard headerTitle="Total Users" total={dashboardStats.totalUsers} currentMonthCount={dashboardStats.usersJoined.currentMonth} lastMonthCount={dashboardStats.usersJoined.lastMonth} />
          ...
          <StatsCard headerTitle="Active Users" total={dashboardStats.userRole.total} currentMonthCount={dashboardStats.userRole.currentMonth} lastMonthCount={dashboardStats.userRole.lastMonth} />
        </div>
      </section>
      ...
      <section className='grid grid-cols-1 lg:grid-cols-1 gap-5'>
        <ChartComponent id='chart-1' primaryXAxis={userXAxis} primaryYAxis={useryAxis} title='User Growth' tooltip={{enable: true}}>
          ...
          <SeriesCollectionDirective>
            ...
            <SeriesDirective dataSource={userGrowth} xName='day' yName='count' type='SplineArea' name='Wave' fill='rgba(71, 132, 238, 0.3)'  border={{width:2, color:'#4784EE'}} />          
          </SeriesCollectionDirective> 
        </ChartComponent>
