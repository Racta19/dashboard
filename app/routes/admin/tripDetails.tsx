...
// params in this case are id that 
export const loader = async ({params}: LoaderFunctionArgs) => {
    ...
    return {trip, allTrips: trips.allTrips.map(({$id, tripDetail , imageUrls}) => ({
        id: $id,
            ...
            <section className="container wrapper-md">
                <header>
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <InfoPill text={`${itinerary?.slice(0,4).map((item) => item.location).join(', ') || ''} day plan`} image="/assets/icons/location-mark.svg" />
                    </div>
                </header>
                ...
                <section className="flex gap-3 md:gap-5 items-center flex-wrap">
                    <ChipListComponent id="travel-chip">
                        <ChipsDirective>
                            {pillItems.filter(pill => pill.text).map((pill, i) => (
            ...
            <section className="flex flex-col gap-6">
                ...
                <div className="trip-grid">
                    {allTrips.map(({id, name, imageUrls, itinerary, interests, travelStyle, estimatedPrice}) => (
                        <TripCard id={id} key={id} name={name} location={itinerary?.[0].location ?? ''} imageUrl={imageUrls[0]} tags={[travelStyle, interests]} price={estimatedPrice} />  
                    ))}
                </div>
            </section>
        </main>
